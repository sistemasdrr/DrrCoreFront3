...
export class EditarPorCobrarComponent implements OnInit {

  loading = false

  idAgentInvoice = 0
  idAgentInvoiceDetails = 0
  price = 0
  requestedName = ""
  procedureType = ""
  shippingDate = ""
  ...
  constructor(public dialogRef: MatDialogRef<EditarPorCobrarComponent>,
    ...
    if(data){
      ...
      this.idAgentInvoiceDetails = data.idAgentInvoiceDetails
  ...
  guardar(){
    Swal.fire({
      ...
      if (result.value) {
        this.loading = true
        this.invoiceService.UpdateAgentInvoiceToCollect(this.idAgentInvoice, this.idAgentInvoiceDetails, this.requestedName, this.procedureType, this.shippingDate, this.price).subscribe(
          (response) => {
