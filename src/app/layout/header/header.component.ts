...
import { AuthSubscriberService } from 'app/authentication/auth/auth-subscriber.service';
  ...
  implements OnInit
{
  ...
  constructor(
    ...
    private authSubscriberService: AuthSubscriberService,
  ...
  ngOnInit() {
    ...
    }else if(auth === null && subscriberUser !== null){
  ...
  logout() {
    ...
    }else if(auth === null && subscriberUser){
      this.subs.sink = this.authSubscriberService.logout().subscribe((res) => {
        if (!res.success) {
